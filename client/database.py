...
class ClientDatabase:
    ...
    def __init__(self, name):
        ...
        self.database_engine = create_engine(f"sqlite:///{os.path.join(path, filename)}", echo=False, pool_recycle=7200, connect_args={"check_same_thread": False})

        # Создаём объект MetaData
        self.metadata = MetaData()

        # Создаём таблицу известных пользователей
        users = Table("known_users", self.metadata, Column("id", Integer, primary_key=True), Column("username", String))

        # Создаём таблицу истории сообщений
        history = Table(
            "message_history", self.metadata, Column("id", Integer, primary_key=True), Column("contact", String), Column("direction", String), Column("message", Text), Column("date", DateTime)
        )

        # Создаём таблицу контактов
        contacts = Table("contacts", self.metadata, Column("id", Integer, primary_key=True), Column("name", String, unique=True))
