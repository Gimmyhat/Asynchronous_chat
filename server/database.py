...
class ServerStorage:
    ...
    def __init__(self, path):
        ...
        users_table = Table(
            "Users", self.metadata, Column("id", Integer, primary_key=True), Column("name", String, unique=True), Column("last_login", DateTime), Column("passwd_hash", String), Column("pubkey", Text)
        )
    ...
    def remove_contact(self, user, contact):
        ...
        # Удаляем требуемое
        self.session.query(self.UsersContacts).filter(self.UsersContacts.user == user.id, self.UsersContacts.contact == contact.id).delete()
        self.session.commit()
